...
class QuestionController
{
    ...
    //method shows form for creation question of first type
    ...
    public function create() {
        if($_SERVER['REQUEST_METHOD'] == "POST") {
            ...
            if((count($keys) > 3) and ($question != null) and ($checkbox != null)) {
                ...
                foreach ($keys as $m) {
                    if(preg_match('/answer(\d+)/', $m, $match)) { //all answers
                        if($checkbox == $match[1]) { //correct answers to DB
